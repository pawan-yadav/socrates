  ...
  module InstanceMethods
    ...
    def subject_fa_scores
      ...
      cce_reports.scholastic.all(select: 'cce_reports.*,exams.subject_id,fa_criterias.fa_group_id',
                                 joins: 'INNER JOIN fa_criterias ON cce_reports.observable_id = fa_criterias.id
INNER JOIN exams ON exams.id = cce_reports.exam_id',
                                 conditions: ['batch_id=?', batch_in_context_id]).group_by(&:subject_id)
          .each do |key, val|
        hsh[key.to_i]={}
        val.group_by(&:exam_id).each do |e_id, e_val|
          hsh[key.to_i][e_id]={}
          e_val.group_by(&:fa_group_id).each do |k, v|
            hsh[key.to_i][e_id][k.to_i] = (v.count > 0 ? (v.sum { |e| e.grade_string.to_f }/v.count) : 0)
          end
        end
      end
      hsh
    ...
    def make_scholastic_report
      ...
      examscores = exam_scores.all(joins: {exam: :exam_group},
                                   conditions: {exam_id: exam_ids.flatten.uniq,
                                                exam_groups: {batch_id: batch_in_context_id}},
                                   include: {exam: :exam_group})
      ...
      sub_fa_scores.each do |subject_id, subval|
        ...
        subval.each do |exam_id, examval|
          ...
          if examscore
            ...
            fa_weight = all_weightages.find { |w| w.cce_exam_category_id ==
                (examscore.exam.exam_group.cce_exam_category_id || 1) and w.criteria_type=="FA" }
            sa_weight = all_weightages.find { |w| w.cce_exam_category_id ==
                (examscore.exam.exam_group.cce_exam_category_id || 1) and w.criteria_type=="SA" }
    ...
    def coscholastic_scores
      hsh={}
      cce_reports.coscholastic.all(select:
                                       'cce_reports.*,observations.observation_group_id,observations.name
AS o_name, observations.sort_order ',
                                   joins: 'INNER JOIN observations ON cce_reports.observable_id = observations.id',
                                   conditions: ['batch_id=?', batch_in_context_id],
                                   order: 'observations.sort_order ASC').group_by(&:observation_group_id)
          .each do |key, val|
            hsh[key.to_i]={}
