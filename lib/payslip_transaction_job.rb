...
class PayslipTransactionJob
  ...
  def perform
    @employee_ids.each do |employee_id|
      ...
      monthly_payslips = MonthlyPayslip.find(:all, conditions: ['employee_id=? AND salary_date = ?',
                                                                employee.id, @salary_date],
                                             include: :payroll_category)
      individual_payslips = IndividualPayslipCategory.find(:all, conditions: ['employee_id=? AND salary_date = ?',
                                                                              employee.id, @salary_date])
      salary = Employee.calculate_salary(monthly_payslips, individual_payslips)

      FinanceTransaction.create(
          title: 'Monthly Salary',
          description: "Salary of #{employee.employee_number} for the month #{I18n.l(@salary_date,
                                                                                     format: :month_year)}",
